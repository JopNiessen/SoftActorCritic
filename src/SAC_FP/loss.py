...
@eqx.filter_jit
@eqx.filter_value_and_grad
def pi_rec_loss_fn(model, obs, control, alpha, v_pred, q1_fn, q2_fn, keys):
    control, log_prob = jax.vmap(model)(obs, control, keys)
    q1_pred = jax.vmap(q1_fn)(obs[:,-1], control)
    q2_pred = jax.vmap(q2_fn)(obs[:,-1], control)
    q_pred = jax.lax.min(q1_pred, q2_pred)
    q_pred = jnp.reshape(q_pred, v_pred.shape)
    advantage = q_pred - v_pred
    advantage = jnp.reshape(advantage, log_prob.shape)
