...
from environments.AbstractClass import AbstractEnv


class Linear_SDI(AbstractEnv):
    def __init__(self, b=1, k=.2, dt=.1, end_time=20, **kwargs):
        ...
        """
        dim = 2
        super().__init__(dt, end_time, dim=dim, **kwargs)

        """System parameters"""
        self.A = jnp.array([[0, 1], [0, -k]])
        self.B = jnp.array([0, b])
        self.v = jnp.array([[.5, 0], [0, .5]])
    ...
    def step(self, control, key=None):
        observation, reward, self.done, _ = self._step(control, key)
